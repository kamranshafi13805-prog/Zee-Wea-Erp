...
const compressImage = async (req, res, next) => {
  ...
  try {
    ...
    const steps = [
      { quality: 80, width: 1200 }, { quality: 65, width: 1000 },
      { quality: 50, width: 800 }, { quality: 35, width: 600 },
      { quality: 20, width: 400 }, { quality: 10, width: 300 },
    ];
    let outputBuffer;
    ...
    if (outputBuffer.length > MAX_SIZE_BYTES) {
      return res.status(413).json({ error: 'Image too large. Even after compression it exceeds 200KB.' });
    }
